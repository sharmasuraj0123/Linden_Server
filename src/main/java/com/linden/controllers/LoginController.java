    ...
    public ObjectStatusResponse<?> login(HttpServletRequest request, @RequestBody Account account) {
        ...
        if (userAccount == null && adminAccount == null) {
            ...
            if (userFromDb == null && adminFromDb == null) {
                ...
                if (userFromDb != null) {
                    return handleUserLogin(new User(account.getEmail(), account.getPassword()), session);
                } else {
                    return handleAdminLogin(new Admin(account.getEmail(), account.getPassword()), session);
                }
            }
        } else {
